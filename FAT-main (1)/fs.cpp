...
FS::cp(std::string sourcepath, std::string destpath)
{
    ...
    std::string name2 = destpath;
    std::string start_cwd = cwd;
    uint16_t start_blk = current_blk;
    dir_entry sourcefile;
    int src_pos;
    int dest_pos;
    int number_of_sub_dir = tmp_enter(sourcepath);
    ...
    else if(number_of_sub_dir == 1)
    {
        std::cout << "cp: " << sourcepath << ": Not a directory" << std::endl;
        return -1;
    }
    else
    {
    ...
    number_of_sub_dir = tmp_enter(destpath);
    if(number_of_sub_dir == -1)
    {
        std::cout << "cp: " << destpath << ": No such file or directory" << std::endl;
        return -1;
    }
    else if(number_of_sub_dir == 1)
    {
        std::cout << "cp: " << destpath << ": Not a directory" << std::endl;
        return -1;
    }
    else
    {
...
FS::mv(std::string sourcepath, std::string destpath)
{
    ...
    if(src_pos + 1)
    {
        if(dest_pos + 1)
        {
            ...
            cwd = start_cwd;
            change_dir();
            rm(name);// Remove file from source directory
        }
        else
        {
            current_blk = start_blk;
...
FS::rm(std::string filepath)
{
    ...
    uint16_t start_blk = current_blk;
    std::string start_cwd = cwd;
    int number_of_sub_dir = tmp_enter(filepath);
    if(number_of_sub_dir == -1)
    {
        std::cout << "create: cannot create file '" << filepath << "': No such file or directory" << std::endl;
    }
...
{
    std::string name = filepath1;
    std::string name2 = filepath2;
    std::string start_cwd = cwd;
    uint16_t start_blk = current_blk;
    dir_entry file;
    int src_pos;
    int dest_pos;
    int number_of_sub_dir = tmp_enter(filepath1);

    if(number_of_sub_dir == -1)
    {
        std::cout << "cp: " << filepath1 << ": No such file or directory" << std::endl;
        return -1;
    }
    else if(number_of_sub_dir == 1)
    {
        std::cout << "cp: " << filepath1 << ": Not a directory" << std::endl;
        return -1;
    }
    else
    {
        src_pos = find_entry(filepath2);
        file = files[src_pos];
        current_blk = start_blk;
        cwd = start_cwd;
        change_dir();
    ...
    number_of_sub_dir = tmp_enter(filepath2);
    if(number_of_sub_dir == -1)
    {
        std::cout << "cp: " << filepath2 << ": No such file or directory" << std::endl;
        return -1;
    }
    else if(number_of_sub_dir == 1)
    {
        std::cout << "cp: " << filepath2 << ": Not a directory" << std::endl;
        return -1;
    }
    else
    {
...
FS::mkdir(std::string dirpath)
{
    ...
    uint16_t start_blk = current_blk;
    std::string start_cwd = cwd;
    int number_of_sub_dir = tmp_enter(dirpath);
    if(number_of_sub_dir == -1)
    {
