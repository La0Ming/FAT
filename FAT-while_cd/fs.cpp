...
int FS::find_entry( std::string path)
{
    ...
    {
        //std::cout << i << std::endl;
        //std::string tmp_file_name = files[i].file_name;
...
int FS::cp(std::string sourcepath, std::string destpath)
{
    ...
    if(last_src_slash != -1)
    {
        ...
        name = sourcepath.substr(last_src_slash+1, sourcepath.size());
    }
    ...
    else
    {
        if (sourcefile.access_rights & READ)
        {
            ...
            if(last_dest_slash != -1){
                cd(new_dest_path.substr(0, last_dest_slash + 1));
                new_dest_path = new_dest_path.substr(last_dest_slash + 1, new_dest_path.size());
            }
...
int FS::cd(std::string dirpath) // TODO: Add return support for file fault error message
{
    ...
    std::string name = dirpath;
    std::string start_cwd = cwd;
    while (name[name.size() - 1] == '/' && name.size() > 1) // Remove all '/' at the end
    {
    ...
    std::string tmp = "..";
    size_t pos = 0;
    while ((pos = name.find(split_token)) != std::string::npos) {
        new_path.push_back(name.substr(0, pos));
        name.erase(0, pos + split_token.length());
    }
    ...
    while (dirpath[dirpath.size() - 1] == '/' && dirpath.size() != 1) // Remove all '/' at the end
    {
        dirpath.erase(dirpath.end());
    }
    //dirpath = name;
    /*
    if((int)end == -1){ // tillagd
        end = name.size();
    }
    int pos = 0;
    ...
    {
        if(end != -1){
            pos = find_entry(dirpath.substr(0, end));
        }
        else{
            pos = find_entry(dirpath);
